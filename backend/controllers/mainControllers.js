...
const registerFreelancer = asyncHandler(async (req, res) => {
    ...
    if(result.length > 0)return res.status(409).send("This email already has an account registered");
...
const updateEmployer = asyncHandler(async (req, res) =>  {
    ...
    const query = "UPDATE `c_gigs_s_up_employer` SET emp_name = ?, emp_comp = ?, emp_fb = ?, emp_insta = ?, emp_linkedin = ?, emp_page = ?, emp_pfp = ?, emp_address = ? WHERE emp_id = ?";
...
const updateFreelancer = asyncHandler(async (req, res) =>  {
    ...
    if (result.length == 0)
        return res.status(409).send("Freelancer not found!");

    const query = "UPDATE `c_gigs_s_up_flancer` SET f_name = ?, f_age = ?, f_school = ?, f_level = ?, f_course = ?, f_portfolio = ?, f_fb = ?, f_insta = ?, f_linkedin = ?, f_twitter = ? WHERE f_id = ?";
